...
5. Reject unknown statuses, stages, teams, duplicate/conflicting facts, future results, non-final rows, unsafe metadata, and secret-bearing URLs.
6. Preserve provider name, `retrieved_at`, source ID, and transformation warnings.
7. Provide preview output before any mutation and require an explicit apply action.
8. Fail closed on contract drift. Never guess a renamed field or stage.
...
- Logs and errors redact credentials and query strings.
