...
def test_update_market_odds_entrypoint_can_be_invoked_directly() -> None:
    root = Path(__file__).resolve().parents[1]
    result = subprocess.run(
        [sys.executable, "scripts/update_market_odds.py", "--help"],
        cwd=root,
        text=True,
        capture_output=True,
        check=False,
    ...
    assert result.returncode == 0, result.stderr
    assert "Validate and merge outright market odds" in result.stdout
