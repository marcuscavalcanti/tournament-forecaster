...
Race-resistant provider apply and durable report publication require POSIX no-follow and directory-descriptor primitives in `v0.1.2`. macOS and Linux are native targets. Native Windows is not supported; Windows operators must use WSL2 and Linux paths.
